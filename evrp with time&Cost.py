...
road_network = load_road_network('newroads.csv')
...
# Function to visualize the route on map
def visualize_route(route, costs, times):
    map = folium.Map(location=[district_points[route[0]]['Latitude'], district_points[route[0]]['Longitude']],
                     zoom_start=8)

    # Plot route with cost and time information
    for i in range(len(route) - 1):
        current_node = route[i]
        next_node = route[i + 1]
        cost = costs[(current_node, next_node)]
        time = times[(current_node, next_node)]
        folium.PolyLine([(district_points[current_node]['Latitude'], district_points[current_node]['Longitude']),
                         (district_points[next_node]['Latitude'], district_points[next_node]['Longitude'])],
                        color="blue", weight=2.5, opacity=1, popup=f"Cost: ${cost:.2f}, Time: {time:.2f} hours").add_to(map)
...
# Calculate cost and time for each road segment
total_cost = 0
total_time = 0
costs = {}
times = {}
for i in range(len(shortest_route) - 1):
    ...
    # Check if current node is different from next node
    if current_node != next_node:
        # Check if edge exists in the road network
        if road_network.has_edge(current_node, next_node):
            distance = road_network[current_node][next_node]['weight']

            cost_per_kwh = charging_stations[district_points[current_node]['ChargingStationID']]['CostPerKWh']
            cost = (vehicle_consumption_rate * distance * cost_per_kwh) / vehicle_battery_capacity
            time = distance / (vehicle_battery_capacity * vehicle_consumption_rate) + charging_time

            total_cost += cost
            total_time += time

            costs[(current_node, next_node)] = cost
            times[(current_node, next_node)] = time
        else:
            print(f"Warning: No road segment found between {current_node} and {next_node}")

print("shortest_rout:",shortest_route)
print("Minimum Cost:", total_cost)
print("Minimum Time:", total_time)


# Visualize the route on map
visualize_route(shortest_route, costs, times)
