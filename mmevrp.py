...
# Function to visualize the route in motion
def visualize_route_in_motion(route):
    ...
    # Plot starting point
    folium.Marker(location=[district_points[route[0]]['Latitude'], district_points[route[0]]['Longitude']],
                  popup="Starting Point",
                  icon=folium.Icon(color='blue')).add_to(map)

    # Animate the path
    ant_path = AntPath(
        locations=[[district_points[node]['Latitude'], district_points[node]['Longitude']] for node in route],
        delay=1000,  # milliseconds
        color='red',
        pulse_color='blue'
    ).add_to(map)

    map.save('route_map_in_motion.html')
    print("Map saved as route_map_in_motion.html")
...
visualize_route_in_motion(shortest_route)